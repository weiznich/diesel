...
where
    Conn: Connection<Backend = DB>, // + IterableConnection<'a, DB>,
    T: AsQuery + RunQueryDsl<Conn>,
    T::Query: QueryFragment<DB> + QueryId,
    T::SqlType: CompatibleType<U, DB>,
    DB: Backend + QueryMetadata<T::SqlType> + 'a,
    U: FromSqlRow<<T::SqlType as CompatibleType<U, DB>>::SqlType, DB> + 'a,
    <T::SqlType as CompatibleType<U, DB>>::SqlType: 'a,
{
    type Ret = LoadIter<
        'a,
        U,
...
where
    ...
    T::Query: QueryFragment<DB> + QueryId,
    T::SqlType: CompatibleType<U, DB>,
    DB: Backend + QueryMetadata<T::SqlType>,
    U: FromSqlRow<<T::SqlType as CompatibleType<U, DB>>::SqlType, DB>,
    for<'a> Self: LoadQueryRet<
        'a,
        Conn,
        U,
        Ret = LoadIter<
