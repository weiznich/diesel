...
use backend::{Backend, RawValue, Ref};
...
/// `FromSql`
impl<DB: Backend<RawValue = Ref<[u8]>>> FromSql<sql_types::Text, DB> for *const str {
    fn from_sql(bytes: Option<RawValue<DB>>) -> deserialize::Result<Self> {
        use std::str;
...
/// raw pointer instead of a reference with a lifetime due to the structure of
/// `FromSql`
impl<DB: Backend<RawValue = Ref<[u8]>>> FromSql<sql_types::Binary, DB> for *const [u8] {
    fn from_sql(bytes: Option<RawValue<DB>>) -> deserialize::Result<Self> {
